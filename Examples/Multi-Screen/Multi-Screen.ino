...
void setup() {
  ...
  Tick0ID = TickO.add(40, 100, "2 db", 2.0);
  Tick1ID = TickO.add(40, 130, "3 db", 3.0);
  Tick2ID = TickO.add(40, 160, "4 db", 4.0);
  Tick3ID = TickO.add(40, 190, "6 db", 6.0);
  TickO.select(Tick1ID);

  SnapO.init(C_WHITE, C_GREEN , C_DKGREY , C_WHITE, BackColor, 20, -2, Arial_14 );
  Snap0ID = SnapO.add(200, 100, "2 db", 2.0);
  Snap1ID = SnapO.add(200, 130, "3 db", 3.0);
  Snap2ID = SnapO.add(200, 160, "4 db", 4.0);
  Snap3ID = SnapO.add(200, 190, "6 db", 6.0);
  SnapO.select(Snap2ID);
...
void Process_SettingsScreen() {
  ...
  while (KeepIn) {
    if (Touch.dataAvailable()) {
      ...
      TickCB.press(BtnX, BtnY);
      TickO.press(BtnX, BtnY);

      SnapCB.press(BtnX, BtnY);
      SnapO.press(BtnX, BtnY);

      if (!TickCB.value) {
        TickSetting = 0.0;
        TickO.disable();
        TickO.draw(TickO.option);
      }
      else {
        TickSetting = TickO.value;
        TickO.enable();
        TickO.draw(TickO.option);
      }
      ...
      if (!SnapCB.value) {
        SnapSetting = 0.0;
        SnapO.disable();
        SnapO.draw(SnapO.option);
      }
      else {
        SnapSetting = SnapO.value;
        SnapO.enable();
        SnapO.draw(SnapO.option);
      }
...
void Process_SetColors() {
  ...
  while (KeepIn) {
    ...
    if (Touch.dataAvailable()) {
      ...
      BackColor = Display.color565(Red.value, Green.value, Blue.value);
